...
void CaptureFih(AppState& state, Status& status) 
{
	...
	while (state.fihing.load() && status != FINISHED)
	{
		...
		case STARTED: //тут  должно быть закидывание удочки
			state.statusMessage = "thrown"; //тут нужно дополнительное состояние чтобы каждый цикл не кидать
			keybd_event(VK_LBUTTON, 0, 0, 0);
		...
		case CATCH:
			...
			keybd_event(VK_LBUTTON, 0, KEYEVENTF_KEYUP, 0);
			// Переключиться на вытаскивание рибы
		...
		case PULL:
			...
			// Логика для контроля положения рыбы
			status = FINISHED;
			break;
		...
		// Короткая задержка для предотвращения 100% CPU
